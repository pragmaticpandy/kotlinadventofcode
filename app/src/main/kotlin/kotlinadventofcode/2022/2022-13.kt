...
class `2022-13` : Day {
    ...
    override fun runPart2(input: String): String {
        ...
        val sortedPackets =
            (parse(input).flatMap { listOf(it.first, it.second) } + divider2 + divider6).sorted()

        return ((sortedPackets.indexOf(divider2) + 1) * (sortedPackets.indexOf(divider6) + 1)).toString()
    }
    ...
    private data class ListNode(val list: List<Node>): Node {
        override operator fun compareTo(other: Node): Int {
            if (other is ListNode) {
                for (i in 0..min(list.lastIndex, other.list.lastIndex)) {
                    val result = list[i].compareTo(other.list[i])
                    if (result != 0) return result
                }

                return if (list.size < other.list.size) -1
                    else if (other.list.size < list.size) 1
                    else 0
            } else {
                return this.compareTo(ListNode(listOf(other)))
            }
        }
    ...
    private data class IntNode(val int: Int): Node {
        override operator fun compareTo(other: Node): Int {
            return if (other is IntNode) {
    ...
    private fun parse(input: String): List<Pair<ListNode, ListNode>> {
        val grammar = object : Grammar<List<Pair<ListNode, ListNode>>>() {
             ...
             */
            val intNode by positiveIntRegex use { text.toInt() } map { IntNode(it) }
            val node by parser(this::listNode) or intNode

            val listNode: Parser<ListNode> by skip(ob) and
                separatedTerms(node, c, acceptZero = true) and skip(cb) map { ListNode(it) }

            val pair by listNode and skip(nl) and listNode map { it.t1 to it.t2 }

            /*
             * Root parser.
             */
            override val rootParser by separatedTerms(pair, 2 times nl)
        }

        return grammar.parseToEnd(input)
    }

    override val defaultInput = """[[[7,8],5],[[9,[8,7,8],[],[2,4,10,10],[2,10,8,3,3]],[],[[6,1,10],[],3,6],[3]],[],[4],[3,0,1,10]]
