...
class `2015-08` : Day {
    ...
    fun parse(input: String): List<String> {
        val grammar = object : Grammar<List<String>>() {
            ...
            val escapedBackslash by literalToken("""\\""")
            val escapedHex by regexToken("\\\\x[0-9a-f]{2}")
            val escapedQuote by literalToken("""\"""")
            val newLine by literalToken("\n")
            val c by regexToken("""[^\n"\\]""")

            val cString by c use { text.toString() }
            val backslashString by escapedBackslash asJust "\\"
            val quoteString by escapedQuote asJust "\""
            val hexString by escapedHex use { Char(text.substring(2, 4).toInt(16)).toString() }
            val line by -quote and zeroOrMore(backslashString or quoteString or hexString or cString) and -quote
            override val rootParser: Parser<List<String>> by
                separatedTerms(line use { joinToString(separator = "") }, newLine)
        }
    ...
    override fun runPart1(input: String): String {
        val codeLength = input.replace("\n", "").length
        val stringsLength = parse(input).sumBy { it.length }
        return (codeLength - stringsLength).toString()
    }
    ...
    override fun runPart2(input: String): String {
        return (input.count { it == '\\' || it == '"' } + 2 + (2 * input.count { it == '\n' }))
                .toString()
    }

    override val defaultInput = """"qxfcsmh"
