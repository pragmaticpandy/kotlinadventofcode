...
class `2015-16` : Day {
    ...
    data class Sue(val number: Int, val characteristics: Map<String, Int>)

    // Sue 2: akitas: 9, children: 3, samoyeds: 9
    fun parse(input: String): List<Sue> {
        val grammar = object: Grammar<List<Sue>>() {

            // Declaration order affects which tokens are tested first. So, match literals first.
            val prefix by literalToken("Sue ")
            val colonSpace by literalToken(": ")
            val commaSpace by literalToken(", ")
            val newline by literalToken("\n")

            // Then match expressions.
            val numberToken by regexToken("""\d+""")
            val nameToken by regexToken("""[a-z]+""")

            // Parsers.
            val number by numberToken use { text.toInt() }
            val name by nameToken use { text }
            val characteristic by name and skip(colonSpace) and number
            val line by skip(prefix) and number and skip(colonSpace) and separatedTerms(characteristic, commaSpace)
            val sue by line.map { Sue(it.t1, it.t2.associate { characteristic -> characteristic.t1 to characteristic.t2 }) }
            override val rootParser by separatedTerms(sue, newline)
        }

        return grammar.parseToEnd(input)
    }

    private fun characteristicFilter(key: String, value: Int): (Sue) -> Boolean {
        return { !it.characteristics.containsKey(key) || it.characteristics[key] == value }
    }
    ...
    override fun runPartOneNoUI(input: String): String {
        return parse(input)
            .filter(characteristicFilter("children", 3))
            .filter(characteristicFilter("cats", 7))
            .filter(characteristicFilter("samoyeds", 2))
            .filter(characteristicFilter("pomeranians", 3))
            .filter(characteristicFilter("akitas", 0))
            .filter(characteristicFilter("vizslas", 0))
            .filter(characteristicFilter("goldfish", 5))
            .filter(characteristicFilter("trees", 3))
            .filter(characteristicFilter("cars", 2))
            .filter(characteristicFilter("perfumes", 1))
            .singleOrNull()
            ?.number
            .toString()
    }
    ...
    override fun runPartTwoNoUI(input: String): String {
        return parse(input)
            .filter(characteristicFilter("children", 3))
            .filter { !it.characteristics.containsKey("cats") || it.characteristics["cats"]!! > 7 }
            .filter(characteristicFilter("samoyeds", 2))
            .filter { !it.characteristics.containsKey("pomeranians") || it.characteristics["pomeranians"]!! < 3 }
            .filter(characteristicFilter("akitas", 0))
            .filter(characteristicFilter("vizslas", 0))
            .filter { !it.characteristics.containsKey("goldfish") || it.characteristics["goldfish"]!! < 5 }
            .filter { !it.characteristics.containsKey("trees") || it.characteristics["trees"]!! > 3 }
            .filter(characteristicFilter("cars", 2))
            .filter(characteristicFilter("perfumes", 1))
            .singleOrNull()
            ?.number
    ...
    override val defaultInput = """Sue 1: cars: 9, akitas: 3, goldfish: 0
Sue 2: akitas: 9, children: 3, samoyeds: 9
