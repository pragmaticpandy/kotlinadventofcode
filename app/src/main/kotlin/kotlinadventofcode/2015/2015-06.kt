...
class `2015-06` : Day {
    ...
    enum class Instruction {
        ...
        companion object {
            fun parse(instruction: String): Instruction {
                return when (true) {
                    instruction.startsWith("turn on") -> ON
                    instruction.startsWith("turn off") -> OFF
                    instruction.startsWith("toggle") -> TOGGLE
                    else -> throw Exception("Instruction not recognized: $instruction")
                }
            }
        }
    }

    fun InstructionForRange(instruction: String): InstructionForRange {
        val trimmedString = instruction.trim { !it.isDigit() }
    ...
    override fun runPart1(input: String): String {
        ...
        defaultInput.lines().map { InstructionForRange(it) }.forEach {
            when (it.instruction) {
