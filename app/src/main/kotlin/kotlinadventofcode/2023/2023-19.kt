...
class `2023-19` : Day {
    ...
    override fun runPartOne(input: String): String {
        ...
        val accepted: MutableSet<Part> = mutableSetOf()
        val workflowsById = workflows.associateBy { it.id }
        parts.forEach("sorting parts") { part ->
            var workflow = workflowsById["in"]!!
            while (true) {
                when (val result = workflow.send(part)) {
                    ...
                    is RedirectResult -> {
                        workflow = workflowsById[result.workflowId]!!
                    }
                    is PassResult -> {
                        error("No workflow should pass")
                    }
                }
            }
        }

        return accepted.sumOf { it.totalRating }.toString()
    }
    ...
    override fun runPartTwo(input: String): String {
        ...
        fun Map<Category, Set<Int>>.numCombinations(): BigInteger {
            return values.map { it.size.toBigInteger() }.reduce(BigInteger::times)
        }

        val acceptableRanges = workflowsById["in"]!!
            .getAcceptableRanges(Category.entries.associateWith { (1..4_000).toSet() }, workflowsById)

        val combinations = acceptableRanges.map { range -> range to range.numCombinations() }
        var result = combinations.sumOf { it.second }

        // remove duplicates
        for (i in 0 until combinations.size - 1) {
            for (j in i + 1 until combinations.size) {
                val (range1, count1) = combinations[i]
                val (range2, count2) = combinations[j]

                val intersectionRange = range1.mapValues { (category, range) ->
                    range.intersect(range2[category]!!)
                }

                result -= intersectionRange.numCombinations()
            }
        }
    ...
    companion object {
        ...
        data class Workflow(val id: String, val rules: List<Rule>) {
            fun send(part: Part): RuleResult {
                for (rule in rules) {
                    val result = rule.sort(part)
                    if (result !is PassResult) return result
                }

                error("No rule matched for $part")
            }

            fun getAcceptableRanges(
                acceptableSoFar: Map<Category, Set<Int>>,
                workflowsById: Map<String, Workflow>): Set<Map<Category, Set<Int>>> {

                val remaining = acceptableSoFar.toMutableMap()
                val result = mutableSetOf<Map<Category, Set<Int>>>()

                fun handleRuleResult(rule: Rule, range: Map<Category, Set<Int>>) {
                    when (rule.ruleResult) {
                        is DecisionResult -> {
                            if (rule.ruleResult.decision == Decision.ACCEPT) {
                                result.add(range)
                            }
                        }

                        is RedirectResult -> {
                            val workflow = workflowsById[rule.ruleResult.workflowId]!!
                            result.addAll(workflow.getAcceptableRanges(range, workflowsById))
                        }

                        is PassResult -> {
                            error("didn't expect a pass here")
                        }
                    }
                }

                rules.forEach { rule ->
                    if (rule.excludedRange == null) {
                        handleRuleResult(rule, remaining)
                        return result
                    } else {
                        val category = rule.category!!
                        val capturedByRule = remaining.toMutableMap()
                        capturedByRule[category] = capturedByRule[category]!! - rule.excludedRange
                        remaining[category] = remaining[category]!! - rule.includedRange!!
                        handleRuleResult(rule, capturedByRule)
                    }
                }

                error("Expected to have returned in the loop")
            }
        }

        data class Rule(val category: Category?, val excludedRange: IntRange?, val ruleResult: RuleResult) {
            val includedRange = excludedRange?.let { (1..4_000).toSet() - it }

            fun sort(part: Part): RuleResult {
                return category?.let {
                    val partVal = when (it) {
                        Category.X -> part.x
                        Category.M -> part.m
                    ...
                    if (partVal in excludedRange!!) PassResult else ruleResult
                } ?: ruleResult
            }
        }
        ...
        data class RedirectResult(val workflowId: String): RuleResult()
        data object PassResult : RuleResult()
        ...
        fun String.parse(): Pair<List<Workflow>, List<Part>> {
            val grammar = object : Grammar<Pair<List<Workflow>, List<Part>>>() {

                // tokens
                val newlineLit by literalToken("\n")
                val colonLit by literalToken(":")
                val commaLit by literalToken(",")
                val gtLit by literalToken(">")
                val ltLit by literalToken("<")
                val leftBrace by literalToken("{")
                val rightBrace by literalToken("}")
                val equalLit by literalToken("=")
                val rLit by literalToken("R")
                val upperALit by literalToken("A")
                val xLit by literalToken("x")
                val mLit by literalToken("m")
                val aLit by literalToken("a")
                val sLit by literalToken("s")
                val lowercaseRegex by regexToken("[a-z]")
                val positiveIntRegex by regexToken("\\d+")

                // parsers
                val accept by upperALit use { Decision.ACCEPT }
                val reject by rLit use { Decision.REJECT }
                val lowercaseChar by xLit or mLit or aLit or sLit or lowercaseRegex use { text }
                val workflowId by oneOrMore(lowercaseChar) map { it.joinToString("") }
                val acceptRuleResult by accept map { DecisionResult(it) }
                val rejectRuleResult by reject map { DecisionResult(it) }
                val redirectRuleResult by workflowId map { RedirectResult(it) }
                val ruleResult by acceptRuleResult or rejectRuleResult or redirectRuleResult
                val positiveInt by positiveIntRegex use { text.toInt() }

                val acceptRule by accept map { Rule(null, null, DecisionResult(Decision.ACCEPT))  }
                val rejectRule by reject map { Rule(null, null, DecisionResult(Decision.REJECT))  }
                val redirectRule by redirectRuleResult map { Rule(null, null, RedirectResult(it.workflowId))  }

                val xCat by xLit map { Category.X }
                val mCat by mLit map { Category.M }
                val aCat by aLit map { Category.A }
                val sCat by sLit map { Category.S }
                val category by xCat or mCat or aCat or sCat

                val greaterThanRule by category and skip(gtLit) and positiveInt  and skip(colonLit) and ruleResult map { (category, value, ruleResult) ->
                    Rule(category, 1..value, ruleResult)
                }

                val lessThanRule by category and skip(ltLit) and positiveInt  and skip(colonLit) and ruleResult map { (category, value, ruleResult) ->
                    Rule(category, value..4_000, ruleResult)
                }

                val rule by greaterThanRule or lessThanRule or acceptRule or rejectRule or redirectRule
                val rules by separatedTerms(rule, commaLit)
                val workflow by workflowId and skip(leftBrace) and rules and skip(rightBrace) map { (id, rules) ->
                    Workflow(id, rules)
                }

                val workflows by separatedTerms(workflow, newlineLit)

                val part by skip(leftBrace) and skip(xLit) and skip(equalLit) and positiveInt and
                    skip(commaLit) and skip(mLit) and skip(equalLit) and positiveInt and
                    skip(commaLit) and skip(aLit) and skip(equalLit) and positiveInt and
                    skip(commaLit) and skip(sLit) and skip(equalLit) and positiveInt and skip(
                    rightBrace
                ) map { (x, m, a, s) ->
                    Part(x, m, a, s)
                }

                val parts by separatedTerms(part, newlineLit)
                override val rootParser by workflows and skip(2.times(newlineLit)) and parts map { (workflows, parts) ->
                    workflows to parts
                }
            }
    ...
    override val defaultInput = """bvh{a>2239:lr,m>657:mqd,x>928:lz,ndn}
