...
class `2023-02` : Day {
    ...
    private data class Game(val id: Int, val draws: List<Draw>) {
        fun possiblyContains(numRed: Int, numGreen: Int, numBlue: Int): Boolean =
            draws.all { draw ->
                draw.counts.all { count ->
                    when (count.color) {
                        Color.RED -> count.count <= numRed
                        Color.GREEN -> count.count <= numGreen
                        Color.BLUE -> count.count <= numBlue
                    }
                }
            }

        fun maximumSeen(color: Color): Int =
            draws.mapNotNull { draw -> draw.counts.find { it.color == color } }.maxOfOrNull { it.count } ?: 0

        fun powerOfMinimumRequired(): Int {
            return maximumSeen(Color.RED) * maximumSeen(Color.GREEN) * maximumSeen(Color.BLUE)
        }
    }

    private data class Draw(val counts: List<ColorCount>) {}

    private data class ColorCount(val color: Color, val count: Int) {}
    ...
    private fun parseGames(input: String): List<Game> {
        val grammar = object : Grammar<List<Game>>() {
            val newlineLit by literalToken("\n")
            val spaceLit by literalToken(" ")
            val commaLit by literalToken(",")
            val semicolonLit by literalToken(";")
            val colonLit by literalToken(":")
            val redLit by literalToken("red")
            val greenLit by literalToken("green")
            val blueLit by literalToken("blue")
            ...
            val game by (skip(gameLit) and skip(spaceLit) and positiveInt and skip(colonLit) and skip(spaceLit) and draws) map { Game(it.t1, it.t2) }

            override val rootParser by separatedTerms(game, newlineLit)
        }

        return grammar.parseToEnd(input)
    }

    override val defaultInput = """Game 1: 4 red, 1 green, 15 blue; 6 green, 2 red, 10 blue; 7 blue, 6 green, 4 red; 12 blue, 10 green, 3 red
