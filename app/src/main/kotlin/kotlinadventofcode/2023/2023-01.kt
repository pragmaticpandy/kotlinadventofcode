...
class `2023-01` : Day {
    ...
    private fun findFirstDigit(input: String): Char {
        return input.first { it.isDigit() }
    }

    private fun findLastDigit(input: String): Char {
        return input.last { it.isDigit() }
    }
    ...
    private fun findFirstDigit2(input: String): Int {
        stringsToInt.forEach() { (key, value) ->
            if (input.startsWith(key))
                return value
        }

        return findFirstDigit2(input.substring(1))
    }

    private fun findLastDigit2(input: String): Int {
        stringsToInt.forEach() { (key, value) ->
            if (input.endsWith(key))
                return value
        }

        return findLastDigit2(input.substring(0, input.length - 1))
    }

    val stringsToInt = mapOf(
        "0" to 0,
        "1" to 1,
        "2" to 2,
        "3" to 3,
    ...
    override val defaultInput = """3fiveone
