...
class `2023-03` : Day {
    ...
    private data class Schematic(val lines: List<String>) {
        ...
        val xIndices = lines[0].indices
        val yIndices = lines.indices
        ...
        fun getGearRatios(): List<Int> {
            return getGearRatios(getPartNums().partNumIdsByIndex)
        }

        fun getGearRatios(partNumIdsByIndex: Map<Pair<Int, Int>, Pair<Int, Int>>): List<Int> {
            val result = mutableListOf<Int>()
            for (y in yIndices) {
                for (x in xIndices) {
                    if (get(x, y) == '*') {
                        val adjacentPartNums = getAdjacentPartNums(x, y, partNumIdsByIndex)
                        if (adjacentPartNums.size == 2) {
                            result.add(adjacentPartNums[0] * adjacentPartNums[1])
                        }
                    }
                }
            }
        ...
        fun getAdjacentPartNums(x: Int, y: Int, partNumIdsByIndex: Map<Pair<Int, Int>, Pair<Int, Int>>): List<Int> {
            val partNumIds = mutableSetOf<Pair<Int, Int>>()

            partNumIdsByIndex[x - 1 to y]?.let { partNumIds.add(it) }
            partNumIdsByIndex[x + 1 to y]?.let { partNumIds.add(it) }
            partNumIdsByIndex[x to y - 1]?.let { partNumIds.add(it) }
            partNumIdsByIndex[x to y + 1]?.let { partNumIds.add(it) }
            partNumIdsByIndex[x - 1 to y - 1]?.let { partNumIds.add(it) }
            partNumIdsByIndex[x + 1 to y - 1]?.let { partNumIds.add(it) }
            partNumIdsByIndex[x - 1 to y + 1]?.let { partNumIds.add(it) }
            partNumIdsByIndex[x + 1 to y + 1]?.let { partNumIds.add(it) }

            return partNumIds.map { getPartNum(it.first, it.second, mutableSetOf(), mutableMapOf())!! }
        }

        fun getPartNums(): SchematicInterpretation {
            val seen = mutableSetOf<Pair<Int, Int>>()
            val partNums = mutableListOf<Int>()
            val partNumIdsByIndex = mutableMapOf<Pair<Int, Int>, Pair<Int,Int>>()
            for (y in yIndices) {
                for (x in xIndices) {
                    if (seen.contains(x to y)) continue
                    val char = get(x, y)
                    if (char?.isDigit() == true) {
                        getPartNum(x, y, seen, partNumIdsByIndex)?.let { partNums.add(it) }
                    }
                }
            }

            return SchematicInterpretation(partNums, partNumIdsByIndex)
        }

        data class SchematicInterpretation(val partNums: List<Int>, val partNumIdsByIndex: Map<Pair<Int, Int>, Pair<Int,Int>>)

        fun getPartNum(x: Int, y: Int, seen: MutableSet<Pair<Int, Int>>, partNumIdsByIndex: MutableMap<Pair<Int, Int>, Pair<Int,Int>>): Int? {
            var isPart = false
            var result = ""
            var currentX = x
            var currentChar = get(currentX, y)
            while (currentChar?.isDigit() == true) {
                result += currentChar
                seen.add(currentX to y)

                // if adjacent is a symbol, then this is part of a part
                if (get(currentX - 1, y)?.isSymbol() == true) isPart = true
                if (get(currentX + 1, y)?.isSymbol() == true) isPart = true
                if (get(currentX, y - 1)?.isSymbol() == true) isPart = true
                if (get(currentX, y + 1)?.isSymbol() == true) isPart = true
                if (get(currentX - 1, y - 1)?.isSymbol() == true) isPart = true
                if (get(currentX + 1, y - 1)?.isSymbol() == true) isPart = true
                if (get(currentX - 1, y + 1)?.isSymbol() == true) isPart = true
                if (get(currentX + 1, y + 1)?.isSymbol() == true) isPart = true

                currentX++
                currentChar = get(currentX, y)
            }

            if (isPart) {
                for (xInPart in x until currentX) {
                    partNumIdsByIndex[xInPart to y] = x to y
                }
    ...
    override val defaultInput = """.......497...........................858...923...128..................227..801........487.....664...........................................
