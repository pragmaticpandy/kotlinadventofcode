...
import kotlinadventofcode.`2023`.`2023-10`.PipeShape.Companion.toShape
import kotlinadventofcode.Day

class `2023-10` : Day {

    override fun runPartOneNoUI(input: String): String {
        return (input.toGrid().loop.size / 2).toString()
    }

    override fun runPartTwoNoUI(input: String): String {
        return input.toGrid().enclosed.size.toString()
    }

    private class Grid(pipeShapes: List<List<PipeShape>>) {
        val pipes: Map<Coordinate, Pipe>
        val start: Pipe
        val xIndices: IntRange
        val yIndices: IntRange

        init {
            this.xIndices = pipeShapes[0].indices
            this.yIndices = pipeShapes.indices
            val pipes = pipeShapes.flatMapIndexed { y, row ->
                row.mapIndexed { x, pipeShape -> Pipe(pipeShape, Coordinate(x, y)) }
            }.associateBy { it.coordinate }.toMutableMap()

            this.pipes = pipes
            this.start = pipes.values.find { it.shape == PipeShape.START }!!
            pipes[start.coordinate] = Pipe(start.adjacent.map { it.second }.toShape(), start.coordinate)
        }

        val loop by lazy { getMainLoop() }
        private fun getMainLoop() = getCycle(listOf(start))

        val enclosed by lazy {
            val result: MutableSet<Pipe> = mutableSetOf()
            for (y in yIndices) {
                var inside = false
                var entryPipeShape: PipeShape? = null
                for (x in xIndices) {
                    val pipe = pipes[Coordinate(x, y)]!!
                    if (pipe in loop) {
                        if (pipe.shape == PipeShape.NORTH_SOUTH) {
                            inside = !inside
                        } else if (pipe.shape in listOf(PipeShape.NORTH_EAST, PipeShape.SOUTH_EAST)) {
                            if (entryPipeShape != null) error("entry pipe already set")
                            entryPipeShape = pipe.shape
                        } else if (pipe.shape == PipeShape.NORTH_WEST) {
                            entryPipeShape!!.let { if (it == PipeShape.SOUTH_EAST) inside = !inside }
                            entryPipeShape = null
                        } else if (pipe.shape == PipeShape.SOUTH_WEST) {
                            entryPipeShape!!.let { if (it == PipeShape.NORTH_EAST) inside = !inside }
                            entryPipeShape = null
                        }
                    } else if (inside) {
                        result += pipe
        ...
        val Coordinate.isInBounds get() = x in xIndices && y in yIndices

        val Coordinate.adjacent
            get() = listOf(
                Coordinate(x, y - 1) to Direction.NORTH,
                Coordinate(x, y + 1) to Direction.SOUTH,
                Coordinate(x - 1, y) to Direction.WEST,
                Coordinate(x + 1, y) to Direction.EAST
            ).filter { it.first.isInBounds }

        val Pipe.adjacent
            get() = coordinate.adjacent.map { pipes[it.first]!! to it.second }
                .filter { it.second in shape.directions }
                .filter { it.first.shape.directions.contains(it.second.opposite) }

        tailrec fun getCycle(path: List<Pipe>): List<Pipe> {
            val nextPipe = path.last().adjacent.map { it.first }.firstOrNull { it !in path } ?: return path
            return getCycle(path + nextPipe)
        }
    }

    private data class Coordinate(val x: Int, val y: Int)

    private data class Pipe(val shape: PipeShape, val coordinate: Coordinate)
    ...
    private enum class PipeShape(val directions: Set<Direction>) {
        ...
        GROUND(setOf()),
        START(setOf(Direction.NORTH, Direction.SOUTH, Direction.EAST, Direction.WEST));

        companion object {
            val shapesByDirection: Map<Set<Direction>, PipeShape> = values().associateBy { it.directions }
            fun Collection<Direction>.toShape() = shapesByDirection[this.toSet()]!!
        }
    }
    ...
    private fun String.toGrid(): Grid {
        val grammar = object : Grammar<Grid>() {
            ...
            val jLit by literalToken("J")
            val sevenLit by literalToken("7")
            val fLit by literalToken("F")
            val dotLit by literalToken(".")
            val sLit by literalToken("S")

            // parsers
            val nsPipe by pipeLit use { PipeShape.NORTH_SOUTH }
            val ewPipe by dashLit use { PipeShape.EAST_WEST }
            val nePipe by lLit use { PipeShape.NORTH_EAST }
            val nwPipe by jLit use { PipeShape.NORTH_WEST }
            val swPipe by sevenLit use { PipeShape.SOUTH_WEST }
            val sePipe by fLit use { PipeShape.SOUTH_EAST }
            val ground by dotLit use { PipeShape.GROUND }
            val start by sLit use { PipeShape.START }
            val pipe by (nsPipe or ewPipe or nePipe or nwPipe or swPipe or sePipe or ground or start)
            override val rootParser by separatedTerms(oneOrMore(pipe), newlineLit) map { Grid(it) }
        }
